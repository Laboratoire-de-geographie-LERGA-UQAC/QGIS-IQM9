...
class IndiceA1(QgsProcessingAlgorithm):
    ...
    def initAlgorithm(self, config=None):
        self.addParameter(
            QgsProcessingParameterRasterLayer(
                self.D8, self.tr("WBT D8 Pointer"), defaultValue=None
            )
        )
        self.addParameter(
            QgsProcessingParameterRasterLayer(
                self.LANDUSE, self.tr("Utilisation du territoir"), defaultValue=None
            )
        )
        self.addParameter(
            QgsProcessingParameterVectorLayer(
    ...
    def processAlgorithm(self, parameters, context, model_feedback):
        ...
        for feature in source.getFeatures():
            ...
            outputs["Watershed"] = processing.run(
                "wbt:Watershed",
                alg_params,
                context=context,
                feedback=feedback,
            ...
            watershed_poly = QgsVectorLayer(
                outputs["PolygonizeRasterToVector"]["OUTPUT"], "poly", "ogr"
            )
            ...
            tot_area = sum(
                [feat.geometry().area() for feat in watershed_poly.getFeatures()]
            )
            forest_area = 0
            agri_area = 0
            ...
            feature.setAttributes(feature.attributes() + [indiceA1])
